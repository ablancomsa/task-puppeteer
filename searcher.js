...
const getNewUsers = async (res) => {
  ...
  const initialization = async () => {
      ...
      await page.goto(`https://www.linkedin.com/search/results/people/?currentCompany=%5B%229826%22%5D&amp;geoUrn=%5B%22104621616%22%5D&amp;origin=FACETED_SEARCH&amp;sid=Awj`);
      ...
      await username.type('msadeveloper@yahoo.com');
      await password.type('proyectolinkedin');

      // PD: CAMBIAR TAMBIÉN LOS ÍNDICES DE ABAJO (indexElement)
      // await page.screenshot({ path: 'src/linkedin/screenshot2.png' }
      // await username.evaluate((node, userData) => node.value = userData.email); //adiaz@msageoconsultores.com 
      // await password.evaluate((node, userData) => node.value = userData.password); //proyectolinkedin
      // PD: CAMBIAR TAMBIÉN LOS ÍNDICES DE ABAJO (indexElement)
      // await page.screenshot({ path: 'src/linkedin/screenshot2.png' });
